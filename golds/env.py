...
ActionsConfig = Union[List[int], Mapping[Asset, List[int]], Tuple[float, float], Mapping[Asset, Tuple[float]]]

Continuity = Enum("Continuity", "CONTINUOUS DISCRETE")
...
class AmericanOptionEnv(Env):
    def __init__(
        ...
        self.observation_space = spaces.Box(-np.inf, np.inf, shape=(len(pricing_source.universe),), dtype=np.float32)
    ...
    def observation_array_to_dict(self, observation: np.ndarray) -> Valuation:
        assert observation.shape == (len(self.pricing_source.universe),)
